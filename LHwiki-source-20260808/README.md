...
“合成潞河”的可维护源码位于 [`games/great-luhe`](games/great-luhe)，发布产物位于
[`public/games/great-luhe`](public/games/great-luhe)。在游戏源码目录运行 `npm ci` 后，
使用 `npm run build` 构建、`npm run qa` 执行浏览器回归；集成与主题通信约定见
[`games/great-luhe/docs/INTEGRATION.md`](games/great-luhe/docs/INTEGRATION.md)。
...
| 版本 | 核心更新 |
| --- | --- |
| **v0.9.1 · 合成潞河娱乐榜** | 个人最好成绩可自愿上传；榜单走缓存和加密对象存储，并以输入校验、双层限流及固定容量控制资源。 |
| **v0.9.0 · 原生样式与多格式导入** | 增加任务项、提示块、代码块和行内样式，统一提供 Markdown、DOCX、LaTeX 和纯文本的本地预检导入。 |
...
- 草稿会即时自动写入当前设备；云端只在用户点击“立即保存”或“提交审核”时以 revision 乐观锁同步，不存在计时上传或失败后的后台重试。支持离线恢复、多页面冲突保护和多个草稿。
