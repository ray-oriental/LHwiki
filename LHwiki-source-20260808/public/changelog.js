export const CHANGELOG_ENTRIES = Object.freeze([
  {
    version: 'v0.9.1',
    title: '合成潞河娱乐榜',
    date: '2026-09-12',
    items: [
      '编辑器将“导入文档”和“导出 Markdown”调整为独立按钮，原有多格式处理能力保持不变。',
      '合成潞河新增自愿上传的娱乐榜：仅在刷新个人最好成绩且合成至少一个潞河后询问，昵称由玩家自行填写。',
      '榜单读取使用浏览器与接口缓存，成绩只写入加密对象存储，不访问 PostgreSQL；写入设有输入校验、多层限流和固定容量。',
      '游戏本体代码与原网址保持不变，外层页面自动适配游戏实际高度，避免出现需要拖动的内嵌滚动条。'
    ]
  },
  {
    version: 'v0.9.0',
    title: '原生样式与多格式导入',
    date: '2026-08-29',
    items: [
      '编辑器新增任务项、提示块、代码块、行内公式和常用行内样式，并统一收入“插入”命令列表。',
      '新增 Markdown、DOCX、LaTeX 和纯文本的浏览器本地导入，应用前显示格式降级警告。',
      '导入可追加或替换，支持撤销/重做；内容协议升级至 schema v3 并同步云端安全白名单。'
    ]
  },
  {
    version: 'v0.8.10',
    title: '工作流存储降耗切换',
    date: '2026-09-09',
    items: [
      '投稿、草稿、审核和权限等私有操作改用加密追加事件，正常网站请求不再访问 PostgreSQL。',
      '原有网址、页面、审核流程、本机自动保存和手动云保存方式保持不变。',
      '并发操作通过逐代条件写入串行提交，网络结果不明时复用同一幂等键。',
      '切换前已对全部现有数据创建加密备份并逐类校验；旧数据库继续只读保留。',
      '公开快照只在私有事件提交成功后发布，普通浏览继续使用静态快照。'
    ]
  },
  {
    version: 'v0.8.9',
    title: '隐式唤醒清零与月度资源预算',
    date: '2026-09-09',
    items: [
      '公开页面启动、普通登入和会话恢复不再访问数据库，减少无人写作时的意外资源消耗。',
      '普通打开写作页直接恢复本机草稿，不再为了查找云端草稿访问数据库。',
      '审核者和管理员执行投稿、审核或管理操作时仍会实时核验权限，原有安全与审核流程保持不变。',
      '编辑内容继续自动保存在本机；云端仅由手动保存、提交及明确管理动作触发，不会后台续传。',
      '资源监控按月度预算和数据库五分钟计费单位判断，保留安全余量并优先区分开发更新与持续异常。'
    ]
  },
  {
    version: 'v0.8.8',
    title: '校园小玩意：合成潞河',
    date: '2026-09-08',
    items: [
      '新增次级“校园小玩意”入口，以不打扰内容共建主线的方式展示“合成潞河”。',
      '游戏完全在当前设备上运行，不访问云函数、数据库、登录或投稿接口；最高分和声音偏好只保存在本机。',
      '游戏会跟随网站的浅色、深色或系统外观，并适配桌面鼠标与移动端触摸。'
    ]
  },
  {
    version: 'v0.8.7',
    title: '手动云端保存与资源唤醒治理',
    date: '2026-09-07',
    items: [
      '编辑内容继续自动保存在本机；只有手动保存或提交审核才会同步云端，不再后台续传。',
      '普通登入和“我的投稿”减少重复数据库与云函数调用，同时保留投稿、审核和权限核验。',
      '访问统计上报永久停用，旧编辑页会在访问数据库前提示刷新。',
      '公开内容只在长缓存未命中时批量读取；审核通过后的新内容可以正常进入公开目录。'
    ]
  },
  {
    version: 'v0.8.6',
    title: '本机写作保护',
    date: '2026-08-23',
    items: [
      '维护期间重新开放完整编辑器，文字编辑、预览和 Markdown 均只在当前浏览器运行。',
      '云端保存与提交继续停用，相关按钮仅保存本机副本并显示维护提示，不访问数据库。',
      '刷新时自动恢复最近的本机草稿，并在离开含有修改的编辑页前请求确认；同时建议另存本机文档或 Markdown。'
    ]
  },
  {
    version: 'v0.8.5',
    title: '资源保护维护模式',
    date: '2026-08-22',
    items: [
      '网站暂时切换为只读维护，公开目录、搜索和文章阅读保持可用。',
      '登入、投稿、云端保存、审核和后台管理暂停，避免继续唤醒数据库。',
      '编辑器继续提供浏览器本地自动保存，并建议另存本机文档或导出 Markdown；此前已成功保存在云端的内容不会消失，恢复后可重新下载。预计恢复日期为 2026 年 9 月 7 日。'
    ]
  },
  {
    version: 'v0.8.4',
    title: '省流写作模式',
    date: '2026-08-22',
    items: [
      '编辑内容继续即时自动保存在本机，云端仅在手动保存或提交时写入。',
      '旧版编辑页面会在保全本机内容后提示刷新，避免后台自动保存继续消耗资源。',
      '云端保存失败后不再后台重试，已有云端草稿和已发布内容不受影响。'
    ]
  },
  {
    version: 'v0.8.3',
    title: '公共浏览降耗与草稿保存保护',
    date: '2026-08-22',
    items: [
      '公共目录、文章和健康检查改用经白名单校验的内容快照，不再因公开浏览唤醒数据库。',
      '长时间写作优先保存在本机，云端自动保存改为较长空闲后合并执行，并在资源繁忙时暂停自动重试。',
      '增加数据库请求预算保护，避免异常调用持续放大资源消耗。'
    ]
  },
  {
    version: 'v0.8.2',
    title: '免费云资源降耗与异常循环保护',
    date: '2026-08-22',
    items: [
      '暂停非必要的访问量统计写入，普通健康检查不再唤醒数据库。',
      '公共目录与文章采用长缓存、并发去重和精简字段读取，显著减少重复 SQL 与传输。',
      '云端草稿改为长间隔合并保存，失败仅有限重试；本机恢复副本仍会即时保留。',
      '取消前后端重试叠乘，并让日常健康检查保持轻量。'
    ]
  },
  {
    version: 'v0.8.1',
    title: '深色模式与 Markdown',
    date: '2026-08-15',
    items: [
      '新增跟随系统、浅色和深色三种外观选择，并记住手动偏好。',
      '投稿编辑器支持 Markdown 输入与输出，兼容现有标题、列表、表格、公式、分栏和折叠内容。',
      'Markdown 链接和代码采用安全文本渲染，不改变数据库结构、云端接口或角色权限。',
      '补全教师卡片、审核队列、致谢头像、投稿提示和更新日志入口的深色视觉层次。'
    ]
  },
  {
    version: 'v0.8.0',
    title: '轻量文档工作台与更新日志',
    date: '2026-08-13',
    items: [
      '编辑器新增四级标题、纯文本表格、分栏、折叠标题、块级公式和分隔线。',
      '统一可搜索的命令面板支持顶部、块旁和斜杠入口，并可使用中英文别名。',
      '完善表格键盘操作、窄屏布局、公式安全渲染、嵌套块标识和高级内容的云端覆盖保护。',
      '新增站内更新日志，并将后续更新同步记录纳入发布流程。'
    ]
  },
  {
    version: 'v0.7.0',
    title: '协作、编辑与资源优化',
    date: '2026-08-12',
    items: [
      '管理员可校订待审核文章，同时保留投稿归属和完整审核流程。',
      '修复回车失焦、长文换行跳顶和搜索失焦，页面滚动会以最小距离跟随输入焦点。',
      '扩充教师索引，并通过缓存、按需加载、访问量批处理和登录限流降低资源消耗。',
      '补齐自动测试、PostgreSQL 迁移与开源发布说明。'
    ]
  },
  {
    version: 'v0.6.0',
    title: '教师索引共建',
    date: '2026-08-09',
    items: [
      '新增教师资料补充入口，资料经独立审核后才进入公开索引。',
      '审核中心和“我的投稿”统一展示文章与教师资料的进度。',
      '加入重复检查、每日上限及对应的数据库权限与备份支持。'
    ]
  },
  {
    version: 'v0.5.1',
    title: '稳定性修复',
    date: '2026-08-09',
    items: [
      '修复 PostgreSQL 时间戳解析和云函数初始化故障。',
      '增加请求超时、只读重试、健康检查、错误降级和双入口巡检。'
    ]
  },
  {
    version: 'v0.5.0',
    title: '写作体验重构',
    date: '2026-08-09',
    items: [
      '上线纯文本块编辑器，支持标题、引用、列表、回车拆分、快捷格式和实时字数。',
      '草稿本地优先保存，再以乐观锁同步云端，并支持离线恢复和冲突保护。',
      '文章可根据标题自动生成两级目录。'
    ]
  },
  {
    version: 'v0.4.0',
    title: '治理、安全与运维',
    date: '2026-08-08',
    items: [
      '新增致谢页和实名内容贡献者记录。',
      '完善管理员保护、已发布文章管理、会话安全、内容校验和访问限流。',
      '建立带校验的自动备份、恢复说明、冒烟测试与稳定性检查。'
    ]
  },
  {
    version: 'v0.3.0',
    title: 'CloudBase 上海迁移',
    date: '2026-08-07',
    items: [
      '生产环境迁至 CloudBase 上海静态托管、Node.js 20 云函数和 PostgreSQL。',
      '配置同域 API 路由，并保留 Cloudflare 版本作为灾备路径。'
    ]
  },
  {
    version: 'v0.2.0',
    title: '校园内容与体验完善',
    date: '2026-08-07',
    items: [
      '加入教师索引、课程评价、校园生活、社团与高三备考等目录。',
      '增加九位学号校内初筛，以及审核者和管理员权限体系。',
      '完成搜索、侧栏导航、移动端布局和经验分享文案。'
    ]
  },
  {
    version: 'v0.1.0',
    title: '最小可用版',
    date: '2026-08-07',
    items: [
      '建立公开目录、分区、文章浏览和关键词搜索。',
      '实现投稿、人工审核和批准后公开发布的完整闭环。',
      '首版运行于 Cloudflare Workers 与 D1。'
    ]
  }
]);

const escapeHtml = value => String(value ?? '').replace(/[&<>'"]/g, char => ({
  '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;'
})[char]);

export function changelogPage() {
  const entries = CHANGELOG_ENTRIES.map((entry, index) => `
    <article class="changelog-entry">
      <div class="changelog-marker" aria-hidden="true"><span>${String(index + 1).padStart(2, '0')}</span></div>
      <div class="changelog-copy">
        <div class="changelog-meta"><strong>${escapeHtml(entry.version)}</strong><time datetime="${escapeHtml(entry.date)}">${escapeHtml(entry.date)}</time></div>
        <h2>${escapeHtml(entry.title)}</h2>
        <ul>${entry.items.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul>
      </div>
    </article>`).join('');

  return `<header class="page-heading changelog-heading"><span class="eyebrow">RELEASE NOTES</span><h1>更新日志</h1><p>记录 LHwiki 从最初版本到现在的功能演进。最新更新排在最前。</p></header><section class="changelog-list" aria-label="LHwiki 版本更新记录">${entries}</section>`;
}
